...
$query="SELECT * FROM assignment where sem=(select student_sem from students where student_usn='$usn') and department='$dep'";
...
while($row = mysqli_fetch_array($result)){
	$td = $td."//$row[1]:$row[2]:$row[7]:$row[8]";	
	$id=$id."//$row[0]";
}
$tr=explode("//",$td);
$id=explode("//",$id);
$querypending="SELECT assignment_id FROM assignment where sem=(select student_sem from students where student_usn='$usn') and department=(select student_department from students where student_usn='$usn') and assignment_id not in(select assignment_id from submitted where student_usn='$usn')";
$resultpend =  mysqli_query($con, $querypending);
$pendid="";
while($rowpend = mysqli_fetch_array($resultpend)){
	$pendid=$pendid."//$rowpend[0]";
}
$pendid=explode("//",$pendid);
			...
			for($r=1;$r<=$rows;$r++){
				echo "<tr>";
					for($d=0;$d<=$cols;$d++){
						...
						$p=$d;
						if($d>=2&&$d!=4){
							echo "<td>".date("M d, Y",strtotime($data[$d]))."</td>";				
						    
						}else if($d==4){
							for($i=1;$i<count($pendid);$i++){
								if($pendid[$i]===$id[$r]){
									$cond=1;
									break;
								}else{
